...
fs.readFileSync("ejFS.html", "utf-8", (err, contenido)=>{ //CAMBIAR EL NOMBRE DEL ARCHIVO PARA PROBAR EL THROW
    if (err) {
        throw err
    }
...
fs.renameSync("ejFS.html", "index.html", (err) => {
    if (err) {
        throw err
    }
...
fs.appendFileSync("index.html", "<p> Etiqueta Agregada </p>", (err) => {
    if (err) {
        throw err
    }
...
fs.writeFileSync("index.html", "Contenido Nuevo en cualquier formato", (err) => {
    if(err){
        throw err
    }
...
/* ELIMINAR ARCHIVOS */
fs.unlinkSync("aEliminar2.html",(err)=>{
    if (err) {
        throw err
    }
